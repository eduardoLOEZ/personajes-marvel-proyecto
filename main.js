...
// Obtener los datos de los personajes de la API y ordenarlos por número en el nombre
fetch("https://api-marvel-v1.onrender.com/marvel/personajes")
  .then((response) => response.json())
  .then((personajes) => {
    personajes.sort((a, b) => {
      const regex = /\d+/; // expresión regular para buscar números
      const aNum = parseInt(a.name.match(regex)[0]); // extraer el número del nombre de A
      const bNum = parseInt(b.name.match(regex)[0]); // extraer el número del nombre de B
      return aNum - bNum; // comparar los números
    });
